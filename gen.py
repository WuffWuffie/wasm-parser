...
for i, ops in enumerate(extra):
    ...
    for start, end in iter_ranges(ops):
        print(
            f"                {start[0]}...{end[0]} => op2{create_offset(start)},",
            file=out,
        )
    print("                else => return error.InvalidOpcode,", file=out)
    print("            },", file=out)
...
# print(
#     "    return @enumFromInt(try @as(error{InvalidOpcode}!u8, switch (op) {", file=out
# )
# for op in base:
#     print(f"        {op[0]} => {fields[op[1]]},", file=out)
# print("        else => error.InvalidOpcode,", file=out)
# print("    }));", file=out)
